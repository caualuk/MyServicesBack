...
async function createUser(req, res) {
  const { name, email, password, phone, role, city_id, profession_id } =
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users 
   (name, email, password, phone, role, city_id, profession_id) 
   VALUES ($1, $2, $3, $4, $5, $6, $7)
   RETURNING *;`,
      [name, email, hashedPassword, phone, role, city_id, profession_id],
    );
...
async function updateProfile(req, res) {
  const { name, profileColor, role } = req.body;
